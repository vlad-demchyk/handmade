...
const ProductsCard = () => {
  ...
  const [cartMemory, setCartMemory] = useState({});
  const [favoriteMemory, setFavoriteMemory] = useState({});

  function addToMemory(type, idItem) {
    if (type === "favorite") {
      setFavoriteMemory((prev) => {
        return {
          ...prev,
          [idItem]: true,
        };
  ...
  function removeFromMemory(type, idItem) {
    if (type === "favorite") {
      setFavoriteMemory((prev) => {
        return {
          ...prev,
  ...
  function changeStateOnButton(type, idItem) {
    if (type === "favorite") return favoriteMemory[idItem] ? "added" : "";
    else if (type === "cart") return cartMemory[idItem] ? "added" : "";
  }

  if (!products) return <div>Loading...</div>; // Показуємо Loading, якщо products ще не завантажені
        ...
        {products.map((item) => (
          <div className="item_card" key={item.id}>
            <div className="image_card">
              <button
                className={`setting-button to_cart ${changeStateOnButton(
                  "cart",
                  item.id
                )}`}
                onClick={() =>
                  cartMemory[item.id]
                    ? removeFromMemory("cart", item.id)
                    : addToMemory("cart", item.id)
                }
              >
                <svg
              ...
              <button
                className={`setting-button to_favorite ${changeStateOnButton(
                  "favorite",
                  item.id
                )}`}
                onClick={() =>
              ...
              >
                ...
                >
                  <path
                    className={`${changeStateOnButton("favorite", item.id)}`}
              ...
              <img src={item.image} alt="product`s image" />
            </div>
